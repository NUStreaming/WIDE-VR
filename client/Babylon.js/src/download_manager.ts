...
export default class DownloadManager {
    ...
    public retrieveNextSegmentForAllObjs = async () => {
        ...
        for (let i = 0; i < objectLevels.length; i++) {
            ...
            downloadPromises.push(this.downloadFromServer(objectLevels[i], this.segmentNo));
        }
    ...
    public downloadFromServer = async (objectToRetrieve: { object: DMeshObject, utility: number, level: number }, frameNo: number) => {
        ...
        const bandwidthKBps = geometryData.byteLength / (endrequest - startrequest);
        SpeedManager.pushBandwidth(bandwidthKBps);
        ...
        StatsLogger.logStatsBySegment(dMeshObject, dMeshSegment);
