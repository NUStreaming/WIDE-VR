...
export default class StrategyManager {
    ...
    private _chosenStrategy: ChooseStrategy = ChooseStrategy.Naive_1; // The strategy we will execute

    private BUFFER: number = 2;                  // Download buffer size in seconds
    private HORIZON: number = 2;                 // Used by Greedy1 and Proposed1 algorithms
    ...
    public constructor(cameraManager: CameraManager, downloadManager: DownloadManager, objectManager: ObjectManager) {
     ...
     */
    public setStrategy(strategy: ChooseStrategy) {
     ...
     */
    public getChosenStrategy = (): ChooseStrategy => {
     ...
     */
    public executeStrategy = async (): Promise<Mesh[]> => {
        console.log(this.getChosenStrategyAsString());
        switch (this._chosenStrategy) {
            case ChooseStrategy.Naive_1:
                return this.naive1();
    ...
    private async moq1(): Promise<Mesh[]> {
        // Start downloading the scene using DownloadManager
        // await this.downloadManager.startDownloading();

        return []; // Return imported meshes
    }
     ...
     */
    private async naive1(): Promise<Mesh[]> {
        ...
        const cameraLater: Camera = this.cameraManager.getCameraInTime(this.HORIZON);
        ...
        const visibles: DMeshObject[] = [...visiblesNow, ...visiblesLater.filter( o => !~visiblesNow.indexOf(o) )];  // !~indexOf returns true if element is not in array and false if it is in
            ...
            for (let j = currentObject.currentLevel; j < currentObject.getNumberOfLevels(); j++) {
                if (!currentObject.getLevel(j)) {
                    ...
                    const utility = currentObject.getLevelQuality(j) *
                                     Metrics.calcUtility(currentObject, this.cameraManager.getCamera());
     ...
     * @param object_manager The object manager calling for the method
     */
    private async proposed1(): Promise<Mesh[]> {
                ...
                if (!currentObject.getLevel(j)) {
                    ...
                    if (increment > 0) {
                        ...
                        for (let t = 0; t < 4; t++) {
                            ...
                            const nextCam = this.cameraManager.getCameraInTime(deltaTime);
                        ...
                        // The Camera is a linear prediction of where the player will look at in a specific amount of time
                        //  This camera is used to calculate the predicted utility and then destroyed for memory reasons
                        const nextCam = this.cameraManager.getCameraInTime(t_next);
     ...
     * @param object_manager The object manager calling for the method
     */
    private async greedy1(): Promise<Mesh[]> {
        //  For this strategy, we download the segment (mesh) that will give us the highest utility
     ...
     * @param object_manager The object manager calling for the method
     */
    private async greedy2(): Promise<Mesh[]> {
        ...
        const utilities: Array<{ object: DMeshObject, utility: number, nextLevel: number }> = [];
     ...
     * @param object_manager The object manager calling for the method
     */
    private async uniform2(): Promise<Mesh[]> {
     ...
     * @param object_manager The object manager calling for the method
     */
    private async hybrid2(): Promise<Mesh[]> {
        ...
        const cam: FreeCamera = this.cameraManager.getCamera();
        const visibles: DMeshObject[] = this.objectManager.getVisibleObjects(cam);
        const notVisibles: DMeshObject[] = this.objectManager.getNotVisibleObjects(cam);
        const visibleUtilities: Array<{ object: DMeshObject, utility: number, nextLevel: number }> = [];
        const notVisibleUtilities: Array<{ object: DMeshObject, utility: number, nextLevel: number }> = [];
        let newLevel = false;       // Will remain false if we don't select a level
